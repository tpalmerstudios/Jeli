...
Texture::Texture (const std::string filename)
    : imgWidth (0), imgHeight (0), count (0), size (0), img ()
{
	int nchannels = -1, w, h;
	unsigned char *pixmap =
		stbi_load (filename.c_str (), &w, &h, &nchannels, 0);
	if (!pixmap)
	{
	...
	if (w != h * int (w / h))
	{
		std::cerr << "Error: the texture file must contain N square "
...
uint32_t
Texture::get (const size_t iX, const size_t iY, const size_t texID) const
{
...
std::vector<uint32_t> Texture::getScaledColumn (const size_t texID,
						const size_t texCoord,
						const size_t columnHeight) const
{
